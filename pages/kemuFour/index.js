...
import { SUBJECT_FOUR_COLLECTION, SUBJECT_FOUR_ERROR_NUMBER } from '../../utils/constant';
import ClassicModel from '../../model/classic.js';
const classicModel = new ClassicModel();
import { getSubjectFour } from '../../utils/cache'
const app = getApp();
Page({
  /**
   * 页面的初始数据
  ...
  async _getClassicList() {
    ...
    const { list, total } = await getSubjectFour({ limit: 100000 })
  ...
  _gotoCollection() {
    const collection = getKeyFromStorage(SUBJECT_FOUR_COLLECTION) || {};
  ...
  _gotoError() {
    const errNums = wx.getStorageSync(SUBJECT_FOUR_ERROR_NUMBER)
    console.log(errNums)
    if (errNums > 0) {
      wx.navigateTo({
    ...
    switch (type.currentTarget.id) {
      ...
      default:
        this._getClassicList().then(() => {
          wx.navigateTo({
            url: `/pages/exerciseFour/exerciseFour?type=${type.currentTarget.id}`,
          });
        })
        break;
    }
