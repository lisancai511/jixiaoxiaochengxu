...
import { getSubjectFour, getSubjectFourCollection } from '../../utils/cache'
import { SUBJECT_FOUR_ERROR, SUBJECT_FOUR_RESULT, SUBJECT_FOUR_SUCCESS, SUBJECT_FOUR_TOPIC_INDEX, SUBJECT_FOUR_ERROR_NUMBER, SUBJECT_FOUR_SUCCESS_NUMBER } from '../../utils/constant'
const app = getApp();
...
Page({
  ...
  gotoItem(e) {
    ...
    wx.setStorageSync(SUBJECT_FOUR_TOPIC_INDEX, index)
    this._initSubject(index)
    this.hideModal()

  },
  _getTopicRecord(topicIndex, res) {
    const subjectResult = wx.getStorageSync(SUBJECT_FOUR_RESULT) || {}
    let successNumber = wx.getStorageSync(SUBJECT_FOUR_SUCCESS_NUMBER) || 0
    let wrongNumber = wx.getStorageSync(SUBJECT_FOUR_ERROR_NUMBER) || 0
    ...
    wx.setStorageSync(SUBJECT_FOUR_RESULT, subjectResult)
    wx.setStorageSync(SUBJECT_FOUR_SUCCESS_NUMBER, successNumber)
    wx.setStorageSync(SUBJECT_FOUR_ERROR_NUMBER, wrongNumber)
  ...
  _saveTopicIndex() {
    ...
    wx.setStorageSync(SUBJECT_FOUR_TOPIC_INDEX, topicIndex)
  },
  collectionItem(e) {
    const { topicIndex } = this.data
  ...
  _checkStar(showMsg = false) {
    const { topicIndex } = this.data
    collection = getSubjectFourCollection()
  ...
  async _initAppData() {
    const { list, total } = await getSubjectFour()
    this.setData({
      total
    })
    cacheList = list
    const index = wx.getStorageSync(SUBJECT_FOUR_TOPIC_INDEX) || 0
  ...
  _initErrorAndSuccess() {
    const successNumber = wx.getStorageSync('SUBJECT_FOUR_SUCCESS_NUMBER') || 0
    const wrongNumber = wx.getStorageSync('SUBJECT_FOUR_ERROR_NUMBER') || 0
    this.setData({
      successNumber,
      wrongNumber
    })
  ...
  _renderModal() {
    let { total } = this.data
    let list = []
    const subjectFourResult = wx.getStorageSync(SUBJECT_FOUR_RESULT) || {}
    for (let i = 0; i < total; i++) {
      if (subjectFourResult[i] === true) {
        list.push({
          index: i,
          class: 'success'
        })
